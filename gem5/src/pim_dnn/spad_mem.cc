...
SpadMem::handleRequest(PacketPtr pkt)
{
    ...
    schedule(new EventFunctionWrapper([this, pkt]{ accessTiming(pkt); },
                                      name() + ".accessEvent", true), clockEdge(Cycles(1)));
    return true;
}

void 
SpadMem::sendResponse(PacketPtr pkt)
{
    assert(busy);
...
void
SpadMem::accessFunctional(Addr addr, int size, uint8_t* data, bool isRead)
{
